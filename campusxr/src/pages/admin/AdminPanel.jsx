...
export default function AdminPanel() {
  ...
  const handleDeleteRoom = async (roomId) => {
    if (!selectedRoomsDeptId) return;
    await deleteRoom(selectedRoomsDeptId, roomId);
    await loadRoomsForDept(selectedRoomsDeptId);
  };

  const handleUpdateRoom = async (roomId) => {
    if (!editRoomName.trim() || !selectedRoomsDeptId) return;
    await updateRoom(selectedRoomsDeptId, roomId, { name: editRoomName.trim() });
                        ...
                        <>
                          <td className="p-4">
                            <input
                              ...
                              className="border border-blue-400 rounded-lg px-3 py-1.5 text-sm w-full focus:outline-none"
                            />
                          </td>
                          <td className="p-4">
                            <input
                        ...
                        <>
                          ...
                          <td className="p-4 text-right space-x-3">
                            <button
                              ...
                              onClick={() => { setEditingDeptId(dept.id); setEditDeptName(dept.name); setEditDeptDesc(dept.description || ''); }}
                            >Edit</button>
                            <button
                  ...
                  {editingRoomId === room.id ? (
                    <div className="flex gap-2 mb-2">
                      ...
                      <button
                        ...
                        className="bg-emerald-500 text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-emerald-600"
                      >Save</button>
                      <button
                    ...
                    <h3 className="text-xl font-bold text-gray-900 mb-0.5">{room.name}</h3>
                  )}
